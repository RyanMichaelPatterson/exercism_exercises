...
def letter_grades(highest):
    ...
    return [41 + (index * grade_range) // 4 for index in range(4)]
...
def student_ranking(student_scores, student_names):
    ...
    for index, name in enumerate(student_scores):
        student_string = str(index + 1) + ". " + str(student_names[index]) + ": " + str(student_scores[index])
        print(student_string)
        joint_list.append(student_string)
